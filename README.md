...
| Пунктирная стрелка | Асинхронный запрос |
| R                  | Чтение             |
| W                  | Запись             |
| R/W                | Чтение и запись    |
...
### Media Service - отвечает за загрузку, хранение и выдачу медиафайлов

| Компонент     | Назначение                                        |
| ------------- | ------------------------------------------------- |
| Media Service | Загружает файлы в s3 и ссылки на них в PostgreSQL |
| S3            | Хранит медиафайлы                                 |

---

### Authentication Service - отвечает за аутентификацию и управление сессиями

| Компонент        | Назначение                                                            |
| ---------------- | --------------------------------------------------------------------- |
| Auth Service     | Обрабатывает регистрацию, вход, проверку пользователя и выдачу сессий |
| Redis (sessions) | Хранит активные пользовательские сессии                               |

---

### User Service - отвечает за пользовательскую бизнес-логику

| Компонент                      | Назначение                                                                                                                      |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| User Service                   | Обрабатывает пользовательские действия связанные с основной бизнес-логикой (добавление товара в корзину, просмотр заказов и тд) |
| Redis (aggregated information) | Хранит агрегированную информацию для быстрого доступа, например количество покупок какого-то товара                             |

---

### Item Service - отвечает за работу с основными сущностями

| Компонент          | Назначение                                                                           |
| ------------------ | ------------------------------------------------------------------------------------ |
| Item Service       | Основной сервис                                                                      |
| PgBouncer          | Пул соединений к PostgreSQL, снижает нагрузку от большого количества подключений     |
| Patroni            | Обеспечивает отказоустойчивость PostgreSQL и автоматическое переключение master-узла |
| PostgreSQL (Citus) | Основное хранилище данных                                                            |
| Lookup Service     | Отвечает за быстрый поиск нужного шарда                                              |
| Cassandra          | Lookup-индекс для маршрутизации запросов по шардам                                   |

---

### Stats Service - отвечает за сбор статистических данных

| Компонент        | Назначение                                                        |
| ---------------- | ----------------------------------------------------------------- |
| Kafka            | Принимает события из системы, например просмотры, клики и покупки |
| DWH Loader       | Читает события из Kafka и загружает их в DWH                      |
| ClickHouse (dwh) | Хранилище аналитических данных                                    |
| Stats Aggregator | Считает агрегаты и записывает готовые данные в Redis              |

---

### Recommendation Service - отвечает рекомендацию товаров

| Компонент               | Назначение                                                            |
| ----------------------- | --------------------------------------------------------------------- |
| API composition         | Собирает ответ для пользователя (наполняет информацией из PostgreSQL) |
| Rec Ranker              | Ранжирует кандидатов и формирует порядок рекомендаций                 |
| Redis (recommendations) | Хранит готовые рекомендации для быстрого чтения                       |
| Candidate Generator     | Генерирует кандидатов для рекомендаций и записывает их в Redis        |

---

### Search Service - отвечает за поиск товаров

| Компонент           | Назначение                                                                                                      |
| ------------------- | --------------------------------------------------------------------------------------------------------------- |
| API composition     | Собирает ответ для пользователя (наполняет информацией из PostgreSQL)                                           |
| Search Service      | Выполняет поиск по товарам через Elasticsearch                                                                  |
| Elasticsearch       | Хранит поисковый индекс товаров                                                                                 |
| Search index worker | Обновляет поисковый индекс на основе событий изменения товаров, периодически синхронизирует данные с PostgreSQL |
| Kafka               | Передает события изменения товаров для обновления индекса                                                       |
