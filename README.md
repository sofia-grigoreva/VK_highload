...
| **PostgreSQL (Citus)** | Master-Slave: 1 синхронная + 1 асинхронная реплика.<br>Graceful shutdown:<br>1. Перестаем принимать новый трафик;<br>2. Завершаем текущие запросы;<br>3. Завершаем работу;<br>Схема резервирования: **full backup 1 раз в сутки + WAL каждые 15 минут** |
