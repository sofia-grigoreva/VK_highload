...
|---|---|
| User | Конечный пользователь системы, который отправляет запросы на просмотр товаров, поиск, авторизацию и получение рекомендаций. |
| BGP Anycast | Направляет пользовательский трафик в ближайший или доступный датацентр, обеспечивая отказоустойчивость и минимизацию задержек. |
| CDN | Кэширует и быстро отдает статический контент и медиафайлы пользователям. |
| S3 | Хранит медиафайлы товаров: изображения, видео и другие бинарные данные. |
...
| Auth Service | Обрабатывает регистрацию, вход в систему, проверку сессий и аутентификацию пользователей и продавцов. |
| User Service | Управляет данными пользователей, профилями, связанными сущностями и частью пользовательской бизнес-логики. |
| Media Service | Управляет загрузкой, хранением и выдачей медиафайлов, а также логикой работы с S3. |
| Search Service | Обрабатывает поисковые запросы пользователей и выполняет поиск по индексу Elasticsearch. |
| Recommendation Service | Отдает пользователю готовые рекомендации, получая их из Redis. Используется в online-контуре с минимальной задержкой. |
| Lookup Service | Определяет, в какой шард или хранилище нужно направить запрос, и инкапсулирует логику работы с lookup-таблицей. |
| PostgreSQL (Citus) | Основное OLTP-хранилище для транзакционных данных: пользователи, товары, корзины, заказы, отзывы и другие сущности. |
| Cassandra | Хранит lookup-данные для быстрого определения нужного шарда по вторичному ключу. |
| Hash Service | Выполняет подготовку производных данных, хэшей или вспомогательных агрегатов для других компонентов системы. |
| Redis | Быстрое key-value хранилище для кэшей, промежуточных агрегатов или горячих данных, нужных сервисам в runtime. |
| Recommendation Generator | Периодически рассчитывает рекомендации на основе событий и агрегатов из ClickHouse и записывает готовые результаты в Redis. |
| Redis (recommendations) | Содержит заранее рассчитанные списки рекомендаций для пользователей и сценариев выдачи. Используется Recommendation Service для быстрого ответа. |
...
| Kafka | Передает события между сервисами асинхронно: изменения товаров, пользовательские действия, события для аналитики и индексации. |
| ClickHouse | Хранит события, логи и агрегаты для аналитики, а также используется как источник данных для фонового расчета рекомендаций. |
