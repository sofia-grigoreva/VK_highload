...
| CDN | Кэширует и быстро отдает статику пользователям |
| S3 | Хранит статику |
...
| User Service | Управляет основной бизнес-логикой |
| Media Service | Управляет загрузкой, хранением и выдачей медиафайлов, а также логикой работы с S3 |
| Search Service | Обрабатывает поисковые запросы пользователей |
| Recommendation Service | Отдает пользователю готовые рекомендации |
...
| Hash Service | Выполняет перерасчет агрегированных метрик |
| Redis | Хранение горячих данных |
| Recommendation Generator | Периодически рассчитывает рекомендации на основе событий из ClickHouse и записывает готовые результаты в Redis |
| Redis (recommendations) | Содержит заранее рассчитанные списки рекомендаций |
